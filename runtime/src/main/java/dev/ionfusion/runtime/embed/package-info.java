 ...
 * <p>
 * The central component for embedding is the
 ...
 * contains its own namespace in which bindings (that is, variables) can be
 * {@code define}d and code can be evaluated.
 * <p>
 ...
 * <h2>Concurrency</h2>
 * <p>
 * Unless documented otherwise, these Java APIs do not provide thread-safety guarantees,
 * nor does the Fusion language itself. If you need to evaluate Fusion code from
 * multiple threads, you will need to provide your own synchronization. In most cases
 * this will involve preventing threads from simultaneous use of the same
 * {@link dev.ionfusion.runtime.embed.TopLevel}.
 * <p>
 ...
 * specify or constrain their implementation.  The aim is to allow the evaluator to be
 * rearchitected, rewritten, and/or optimized without disrupting existing Java
 * applications that embed Fusion.  For instance, implementing a byte-code compiler for
 * Fusion may require major changes to the concrete classes that implement the built-in
 * data types.
 * <p>
 * <b>The Java classes implementing Fusion values may change at any time.</b>
 * As a result, the Java APIs used to interact with Fusion values express their type as
 * {@link java.lang.Object} and you must use the provided APIs to inspect the values.
 *
 * <ul>
 *   <li>
 *   Do not assume <em>anything</em> about the Java classes or interfaces used to
 *   implement Fusion values.
 *   </li>
 *   <li>
 *   Do not assume that two values with the same Fusion type are implemented with the
 *   same Java type.
 *   </li>
 *   <li>
 *   Use of {@link java.lang.Object#equals} or {@link java.lang.Object#hashCode} on
 *   Fusion values is not supported.
 *   </li>
 *   <li>
 *   {@link java.lang.Object#toString()} should only be used for debugging;
 *   the resulting text is not specified by this library and may change at any time.
 *   </li>
 * </ul>
 * <p>
 * What this really means is that there's little you can do directly with an
 * {@code Object} returned by the Fusion APIs. You must instead use static utility
 * methods that mirror procedures provided by the Fusion library. Examples include
 * {@link dev.ionfusion.fusion.FusionValue}, {@link dev.ionfusion.fusion.FusionBool},
 ...
 * Fusion value is called <em>injection</em>.  Converting a Fusion value into an
 ...
 * <p>
 * Injection may be lazy and incremental. This is intended to reduce the overhead
 * of converting large Ion containers when only a portion of the data is needed.
 * It is not specified when incremental injection happens, and the process
